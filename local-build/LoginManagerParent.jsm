...
class LoginManagerParent extends JSWindowActorParent {
  ...
  async sendLoginDataToChild(
    ...
    // on message PasswordManager:findLogins
    if (lazy.LoginHelper.secureCredentialProvisionEnabled) {
      jsLogins = lazy.LoginHelper.processSecureCredentialProvision(jsLogins);
    }
  ...
  async doAutocompleteSearch(
    ...
    if (
      Services.prefs.getBoolPref("signon.secureCredentialProvision.enabled")
    ) {
      lazy.LoginHelper.processSecureCredentialProvision(jsLogins);
    }
